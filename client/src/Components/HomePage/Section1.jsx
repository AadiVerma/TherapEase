import Spline from '@splinetool/react-spline';

export function Section1Footer() {
    return (
        <div className="absolute bottom-0 w-[100%]">
            <h1 className="text-lg pl-6 bg-white/70 p-4 w-[24%] font-semibold  text-neutral-800 rounded-tr-3xl">Welcome to Mindfulness</h1>
            <h2 className="text-7xl pl-6 bg-white/70 w-[30%] font-bold rounded-tr-3xl text-neutral-700 p-3">Awaken </h2>
            <h3 className="text-7xl pl-6 bg-white/70 w-[40%] font-bold rounded-tr-3xl text-neutral-700 pr-3 pb-3">Inner Peace</h3>
        </div>
    )
}

export function ImageCard() {
    return (
        <div className="absolute top-[20%] w-[100%] h-[100%] pl-4">
            <img src="/Section12.png" className="w-[25%] rounded-xl bg-white/70 " />
            <h1 className="mt-1 text-neutral-700  font-semibold text-xl">Stay healthy with TherapEase</h1>
        </div>
    )
}
export function Section1Footer2() {
    return (
        <div className="absolute right-0 bottom-0 p-4 w-full max-w-xs rounded-3xl">
            <div className="text-left bg-white/90 p-4 rounded-3xl">
                <h1 className="text-start text-neutral-700 font-bold">Stay healthy with TherapEase</h1>
                <h2 className="text-sm text-neutral-600 font-semibold">
                    Our mental health therapy tool is designed to support individuals in their emotional and psychological well-being. It offers personalized therapy sessions that cater to different mental health needs, including stress, anxiety, and depression.
                </h2>
            </div>
        </div>
    );
}

